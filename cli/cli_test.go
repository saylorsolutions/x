...
func TestCommandSet_Exec(t *testing.T) {
	set := NewCommandSet()
	require.ErrorIs(t, set.Exec(nil), ErrUnknownCommand)
	...
	require.ErrorIs(t, set.Exec([]string{"Does", "not", "exist"}), ErrUnknownCommand)
}
...
func TestCommand_AddSubCommand(t *testing.T) {
	...
	require.NoError(t, cmd.Exec([]string{"-h"}))

	cmd = testCommandWithSubcommand(t, &cmdExecuted, &subExecuted)
	require.NoError(t, cmd.Exec([]string{"blah"}), "Should execute test without error")
	assert.Equal(t, 1, cmdExecuted)
	assert.Equal(t, 0, subExecuted)

	cmd = testCommandWithSubcommand(t, &cmdExecuted, &subExecuted)
	require.NoError(t, cmd.Exec([]string{"SUB"}))
	assert.Equal(t, 1, cmdExecuted)
...
func TestCommandSet_AddCommand_Aliases(t *testing.T) {
	...
	require.NoError(t, cmd.Exec([]string{"test", "a"}), "Should execute test without error")
	assert.Equal(t, 0, cmdExecuted)
	assert.Equal(t, 1, subExecuted)

	cmd = testCommandSet(t, &cmdExecuted, &subExecuted)
	require.NoError(t, cmd.Exec([]string{"test", "b"}), "Should execute test without error")
	assert.Equal(t, 0, cmdExecuted)
